...
- problemPhrasings: the searches this product's buyers would type, in their own words. Two kinds. First, 4 to 6 short problem statements, each 4 to 8 words, said the way a person says it out loud rather than as a bag of keywords: keep the small words that make it a sentence, and keep the constraint that makes it this product's problem - an age, a limit, a refusal, a negation. Spread them across the distinct situations the page implies rather than rewording one: the occasion the problem arrives with (a trip, an event, a visit), who is acting for whom (a parent arranging for their child), and the moment it bites (a booking already made, a refusal at the desk). For a site listing hotels that check in guests under 21, they would be: "hotels that allow 18 year olds", "under 21 hotel check in", "hotel refused check in because of age", "booked a hotel then found the 21 rule", "parent booking a hotel for an 18 year old". Second, the name a buyer types for the thing itself, one for each system, platform, site or kind of data the page says this product works with or covers: the noun they would search for, not a complaint, as in "reddit scraper", "twitter api alternative", "google maps scraper". Name that platform or system, because it is the word the buyer types. None of this second kind when the page names no such system. In both kinds leave out prices, dates, personal details, city and country names, this product's own name and the names of its rivals: a rival is not a platform, however often the page compares itself to one.
